...
public class RawMaterialConversionFormula {
    ...
    public BigDecimal evaluate(String expression, BigDecimal length, BigDecimal width,
                               BigDecimal thickness, BigDecimal squareMeterWeight) {
        return new Parser(expression, false, length, width, thickness, squareMeterWeight).parse();
    }

    public void validate(String expression) {
        new Parser(expression, true, BigDecimal.ONE, BigDecimal.ONE, BigDecimal.ONE, BigDecimal.ONE).parse();
    }
    ...
    private static final class Parser {
        ...
        private Parser(String expression, boolean validationOnly, BigDecimal length, BigDecimal width,
                       BigDecimal thickness, BigDecimal squareMeterWeight) {
            this.expression = expression == null ? "" : expression;
            this.validationOnly = validationOnly;
            this.length = zero(length);
            this.width = zero(width);
            this.thickness = zero(thickness);
            this.squareMeterWeight = zero(squareMeterWeight);
        }
