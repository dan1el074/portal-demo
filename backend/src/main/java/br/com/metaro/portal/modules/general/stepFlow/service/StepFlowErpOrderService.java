...
public class StepFlowErpOrderService {
    ...
    public ErpOrderDto findAvailableOrderByNumber(int orderNumber) {
        ErpOrderDto orderFromErp = erpOrderQueryService
                .findProductionOrderByNumber(orderNumber)
                .orElseThrow(ResourceNotFoundException::new);
        List<Order> localOrders = orderRepository.findByNumber(
                orderNumber,
    ...
    private void addLocallyProducedQuantities(
    ...
    private void ensureOrderHasPendingProduction(ErpOrderDto orderFromErp) {
        ...
        if (fullyProduced) {
            throw new UnprocessableEntityException(
                    "Pedido não disponível para produção!"
            );
        }
    }
}
